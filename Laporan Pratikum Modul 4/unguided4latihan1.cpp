...
struct mahasiswa
{
string nama;//Field nama berƟpe data string
string nim;//Field nim berƟpe data integer
};
//Struct node terdapat 2 field
struct node
{
mahasiswa ITTP;// field ITTP berƟpe inisialisasi struct mahasiswa
node *next;// Field next berƟpe pointer node
};
//inisialisasi
...
void insertTengah(mahasiswa idenƟtas, int posisi)
{
node *baru = new node;
baru->ITTP.nama = idenƟtas.nama;
baru->ITTP.nim = idenƟtas.nim;
node *bantu;
